...
//		W5500_RST_HIGH();
    ...
    spi_init_struct.prescale             = SPI_PSC_16;								//æœ€å°SPI_PSC_16 ;			168/16=10M
...
void ADS1247_Init(void)
{
	...
	ucReadData=ADS_RREG(D_Reg_CFC0,0x01);			//åç§»æ ¡å‡†ç³»æ•°1  é»˜è®¤0
	ofc = ucReadData;
	
	ucReadData=ADS_RREG(D_Reg_CFC1,0x01);			//åç§»æ ¡å‡†ç³»æ•°2  é»˜è®¤0
	ofc = ofc | (ucReadData<<8);
	
	ucReadData=ADS_RREG(D_Reg_CFC2,0x01);			//åç§»æ ¡å‡†ç³»æ•°3  é»˜è®¤0
...
//	ucReadData=ADS_RREG(D_Reg_VBias,0x01);
	...
	//++++++++++++++++++++++++++++++++++++++++++++++++++  pga=1
	ucReadData=ADS_RREG(D_Reg_SYS0,0x1);
	ADS_WREG(D_Reg_SYS0,ADS1247_PGA_1|ADS1247_DARATE_20);					//PGA=1; ç”µå‹æ”¾å¤§å€æ•° ï¼› data output rate=1000SPS	//ADS_WREG(D_Reg_SYS0,ADS1247_PGA_1|ADS1247_DARATE_40);					//PGA=1; ç”µå‹æ”¾å¤§å€æ•° ï¼› data output rate=1000SPS
	ucReadData=ADS_RREG(D_Reg_SYS0,0x1);
