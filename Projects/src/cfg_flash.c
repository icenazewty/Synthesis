...
#else
#define RJ45_START_ADDR  					0x8040000+0x2000*31				//gs_SaveNetIPCfg		 	RJ45网络参数   32*8
#define	WIFI_START_ADDR   				0x8040000+0x2000*28				//gs_SaveWifiCfg			wifi网络参数

#define ADS1247_START_ADDR 				0x8040000+0x2000*30				//g_Ads1247_Cali			ads1247校准参数
#define PARAM_START_ADDR  				0x8040000+0x2000*29				//g_configRead  			公共参数	
#endif
...
{	
	...
	gs_SaveNetIPCfg.baud[0] = 9600;				//com1  rs485-1  COM1  通讯口android
	gs_SaveNetIPCfg.baud[1] = 9600;				//Com2  rs485-2  COM2  
	gs_SaveNetIPCfg.baud[2] = 115200;			//com3           WIFI  
	gs_SaveNetIPCfg.baud[3] = 9600;				//com4  rs485-3  COM4  
	gs_SaveNetIPCfg.baud[4] = 9600;				//com5           Meter   
		
	gs_SaveNetIPCfg.chekbit[0] = 0;				//0=none  1=奇  2=偶  
	...
	//-----------------------add 2023-12-3----------
	gs_SaveNetIPCfg.baud2[0] = 115200;				//com6  				 4G  
	gs_SaveNetIPCfg.baud2[1] = 9600;					//com7  rs485-4  COM3  
	gs_SaveNetIPCfg.baud2[2] = 9600;					//com8  rs485-5  COM5   
	gs_SaveNetIPCfg.baud2[3] = 9600;					//com9  
	gs_SaveNetIPCfg.baud2[4] = 9600;					//com10  备用   
	
	gs_SaveNetIPCfg.chekbit2[0] = 0;				//0=none  1=奇  2=偶  
	gs_SaveNetIPCfg.chekbit2[1] = 0;
	gs_SaveNetIPCfg.chekbit2[2] = 0;
	gs_SaveNetIPCfg.chekbit2[3] = 0;
	gs_SaveNetIPCfg.chekbit2[4] = 0;	
	
}

void  WriteParametersNetDefault(void)
{
	  unsigned char temp[512],*p;	 
		int  i,j;
...
unsigned char  init_config_net(unsigned char flag)
{
  ...
  if(gs_SaveNetIPCfg.ucStartFlag=='*' && gs_SaveNetIPCfg.ucEndFlag=='#' )//读取成功
	{
		  for(int i = 0 ; i < 5;i++)
		  {
				if(0xff==gs_SaveNetIPCfg.chekbit2[i])
				{
					gs_SaveNetIPCfg.chekbit2[i] = 0;
				}
				if(0xffffffff==gs_SaveNetIPCfg.baud2[i])
				{
					if(0==i)
						gs_SaveNetIPCfg.baud2[i] = 115200;
					else
						gs_SaveNetIPCfg.baud2[i] = 9600;
				}
		  }
			return 1;
	}
