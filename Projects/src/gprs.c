...
int  CmdExe(char*cmd,int mode,unsigned char Para_ucType_Length_Cal,unsigned short Para_usLength)
{
	...
	if(Para_ucType_Length_Cal==0){
		cmd_len= strlen(cmd);	
	}else{
		cmd_len=Para_usLength;
	}
...
//AT+CIPOPEN=0,"TCP","139.129.17.114",7107
	...
	{		
		if(Para_ucType_Length_Cal==0)
		{
			sprintf(data,"4G send:%s\r\n",cmd);		
			InsertLog(data);			
		}
		else
		{					
			sprintf(data,"4G send:%04d  ",cmd_len);
			dw = 7;
			for(i=0;i<Para_usLength ;i++)
			{
				sprintf(data+2*dw,"%02X",cmd[i]);
				if(0==((dw+1)%128))
				{
	...
	//Com.Usart[G4].usRec_RD = Com.Usart[G4].usRec_WR=0;		//Com.Usart[G4].usRec_WR;	
	...
	for(i=0;i<delay_ms&&g_res<512;i++)							//�ִ��2��,������512�ֽ����ݡ�
	{
		...
		if(dw>0)
		{
			...
			g_ComBuf[g_res] = '\0';
//-------------------- MQTT接收判断开始 -------------------------------------------------------------------------------------
			if(Para_ucType_Length_Cal==1)
			{
				if( (g_ComBuf[0]==0x20) && (g_ComBuf[1]==0x02) )	//连接返回  
				{
//					delay_1ms(10);
					success = 1;
					break;
				}
				else if( (g_ComBuf[0]==0x40) && (g_ComBuf[1]==0x02) )//PUBLISH返回  注册 和  数据通讯
				{
//					delay_1ms(10);
					success = 1;
					#if OLD_MDOE
				...
				else if( (g_ComBuf[0]==0xD0) && (g_ComBuf[1]==0x00) )//心跳返回
				{
//					delay_1ms(10);
					success = 1;
					#if OLD_MDOE
  ...
  wdt();
	...
	{	
			...
			{				
				if(Para_ucType_Length_Cal==0)
				{
					sprintf(data,"4G recv:%s\r\n",g_ComBuf);
				}
				else
				{			
					sprintf(data,"4G recv:");				//8 character
					for(i=0;i<g_res && i<250;i++)
					{
						sprintf(data+8+2*i,"%02X",g_ComBuf[i]);
					}
					sprintf(data+8+2*i,"\r\n");
				}
				InsertLog(data);
...
//	DCB		commParam;
	...
	//ģ���Ƿ����ͨѶ->�ֻ����Ƿ����->�ź�����->ע������		�ϼ�4������
		...
		if(!CmdExe(AT_CMD,0,0,0))							//AT����		�ж�ģ���Ƿ��������ҿ���ͨѶ			
		{
		...
		if(!CmdExe(ATE_CMD,0,0,0))					//�رջ���		�ж�ģ���Ƿ��������ҿ���ͨѶ
		{
			InsertLog("ATE command failed!\r\n");				
		}
		
		if(CmdExe(AT_CSQ,0,0,0)==1)					//хۅ׊			Ɛ׏хۅǿ׈ɧێ
		{
				GetCSQ();										//ܱȡхۅǿ׈			
		...
		if(CmdExe(AT_CIMI,0,0,0)==1)			//IMSI			�ж��ֻ����Ƿ����
		{
				GetIMSI();
		...
		if(CmdExe(AT_ICCID,0,0,0)==1)		//iccid				�ж��ֻ���iccid mg323��֧��
		{
		...
		if(CmdExe(AT_COPS,0,0,0)==1)	//gprsθçעӡԉ٦	Ɛ׏θçˇرעӡԉ٦
		{
		...
		if(CmdExe(AT_CREG,0,0,0)==1)	//gsm����ע��ɹ�		�ж������Ƿ�ע��ɹ�
		{
			...
			else
			{
				...
				//return -13;											//中国电信测试 返回 0 ,  3不正确，但有钱
			}
		}
		else
		{
		...
		if(CmdExe(AT_CGREG,0,0,0)==1)	//gprs����ע��ɹ�	�ж������Ƿ�ע��ɹ�
		{
		...
		if(CmdExe(AT_CIPSENDMODE,0,0,0)==1)			//tcp client send ack应答模式
		{
		...
		if(CmdExe(AT_CIPMODE,0,0,0)==1)				//tcp 透传模式
		{
		...
		if(CmdExe(AT_NETOPEN,0,0,0)==1)				//连接网络,获取本地ip地址
		{
		...
		if(CmdExe(AT_IPADDR,0,0,0)==1)					//连接网络后,获取本地ip地址
		{
...
int  OpenGSM(void)
{
	...
	//ģ���Ƿ����ͨѶ->�ֻ����Ƿ����->�ź�����->ע������->�����йصĲ�������(CMGF,CNMI,CSCA)		�ϼ�5������
		...
		if(CmdExe(AT_COPS,0,0,0)==1)	//ܱȡՋӪʌ
		{
			GetCOPS();					//Ɛ׏ˇرӑޭעӡԉ٦θç,ɧڻθçעӡԉ٦ìղƐ׏ؾϱǷʌ			
		}
		else
		...
		//**************************5 �������� bRegGSM  service   ********************	
		if(CmdExe(AT_CMGF,0,0,0)==1)		//PDU��ʽ���Ͷ���
		{
		...
		if(CmdExe(AT_CNMI,0,0,0)==1)	//���Ŵ洢ģʽ����
		{
		...
		if(CmdExe(AT_CSCA,0,0,0)==1)	//��ȡ�������ĵ绰����
		{
...
int  SendSMS(char* PhoneNum,char* Content,int len)
{
		...
		CmdExe(cmd,1,0,0);										//����ƥ�䡮>���ɹ��񶼷�������
		delay_1ms(200);
		nPduLength = CmdExe(pdu,2,0,0);				//״̬1�ɹ�,2ʧ��,0û�л���	��ȴ�30��
		if(1==nPduLength)
		{	
			nPduLength = 1;
...
int ENTRANS(void)
{
	...
	int success = 0;
	success = CmdExe(AT_IPENTRANS,4,0,0);				//进入透传模式  等待3秒  3秒-->5秒
...
//		char AT_QIOPEN[]  	= {"AT+CIPOPEN=0,\"TCP\",\"139.129.17.114\",7106\r\n     "};		//AT+QIOPEN=1,0,"TCP","121.42.176.145"",71070,0,2
		char AT_QIOPEN[]  	= {"AT+CIPOPEN=0,\"TCP\",\"139.129.17.114\",6601\r\n     "};		//AT+QIOPEN=1,0,"TCP","121.42.176.145"",71070,0,2
		char temp[32];
		char temp2[32];
		int	 len  = 0;
		int	 len2  = 0;
    ...
    AT_QIOPEN[24+len+len2]='\0';	
		...
		success = CmdExe(AT_QIOPEN,4,0,0);			//等待3秒时间，OK=1  ERROR=2  0=timeout		3->4  即3秒修改为5秒				0=timeout		1=CONNECT		2=ERROR
    ...
    AT_SISS3[33+g_configRead.IPLen+g_configRead.PortLen]='\0';	//15+5+33
		...
		if(CmdExe(AT_SICS1,0,0,0)!=1)				
		{			
			InsertLog("AT_SICS1 command failed!\r\n");	
			return -20;  
		}
		delay_1ms(5);
		if(CmdExe(AT_SICS2,0,0,0)!=1)				
		{			
			InsertLog("AT_SICS2 command failed!\r\n");	
			return -21;  
		}
		delay_1ms(5);
		if(CmdExe(AT_SISS2,0,0,0)!=1)				
		{			
			InsertLog("AT_SISS2 command failed!\r\n");	
			return -22;  
		}
		delay_1ms(5);
		if(CmdExe(AT_SISS3,0,0,0)!=1)					
		{			
		...
		Com.Usart[G4].usRec_RD=Com.Usart[G4].usRec_WR;			//由于AT_SISO可能需要异步,所以对buf清空。准备后面的异步判断(返回超时)。
		success = CmdExe(AT_SISO,4,0,0);			//等待3秒时间，OK=1  ERROR=2  0=timeout		3->4  即3秒修改为5秒
		if(0==success)					//0=timeout
		{			
...
int CloseGprs(void)
{
		...
		success = CmdExe(AT_IPENTRANS_EXIT,4,0,0);			//5秒退出
		if(success==1)		//成功
		{		
				m_gprsinfo.g_bGPRSConnected = 0;
				Led_Ctrl(0,false);
				success2 = CmdExe(AT_CMD,0,0,0);			//目的为了发\r\n
				delay_1ms(20);
				success = CmdExe(AT_SISC,5,0,0);			//断开连接最长可以等10秒
				g_tranflag = 1;
				if(success==0)
				{
		...
		else							//不成功
		{
				...
				success2 = CmdExe(AT_CMD,0,0,0);			//目的为了发\r\n
				delay_1ms(20);
				success = CmdExe(AT_SISC,5,0,0);			//断开连接最长可以等3秒
				g_tranflag = 1;				
				if(success==0)
				{
...
unsigned char IsDisconn()
{
	...
	char AT_IPENTRANS_EXIT[]   = {"+++"};		
	char AT_SISC[] 	= {"AT+CIPCLOSE=0\r\n"};	
	if(1==CmdExe(AT_CMD,0,0,0))
	{
		return 1;
	}	
	else if(1==CmdExe(AT_CMD,0,0,0))
	{
		return 1;
	}
	else	
...
int Get_ICCID(void)
{
	...
	{
		//**************************1  ģࠩˇرࠉӔͨѶ  bComm  CommFailCnt ***********************************
		if(!CmdExe(AT_CMD,0,0,0))							//ATļ®		Ɛ׏ģࠩˇرֽӣҢȒࠉӔͨѶ			
		{
		...
		if(!CmdExe(ATE_CMD,0,0,0))					//ژҕܘД		Ɛ׏ģࠩˇرֽӣҢȒࠉӔͨѶ
		{
			InsertLog("ATE command failed!\r\n");				
		}
		...
		if(CmdExe(AT_CIMI,2,0,0)==1)			//IMSI			Ɛ׏˖ܺߨˇرզ՚
		{
				GetIMSI();
				m_siminfo.bExist=true;	
		}
		...
		if(CmdExe(AT_ICCID,2,0,0)==1)		//iccid				Ɛ׏˖ܺքiccid mg323һ֧Ԗ
		{
...
//   {
		 if(1==g_configRead.b_gprs_work)
		 {
				...
				if(m_gprsinfo.g_bGPRSConnected)
				{
					//请求建立连接
