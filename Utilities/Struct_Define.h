...
typedef struct CONFIG// 最多只能256字节  当前结构体占用244字节 。CPU内部flash最多2048字节
{
 ...
 //  	3=wifi 	路由器模式，开机默认不工作，可以通过按键来设置是否工作。用于数据导出和配置信息等。
  ...
  short   int    		HMin2;						//通道1湿度下限
	//----------------------------------------2023-12-02
	unsigned char			b_success2;				//如果不为 '!' 则表示新增加的
	unsigned char 		b_4g_apn_enable;	
	unsigned char			g4_apn[24];				
	unsigned char			g4_psw[24];				
	unsigned char			site_name[32];		
} ConfigInfo;
...
typedef struct
{
   ...
   float					pv_i;									//光伏电流
	...
	unsigned char				site_name[32];				//站点名称
...
typedef struct  
{
	unsigned short check;		//0表示无校验，1表示奇校验，2表示偶校验。
	unsigned short baud;		//  *100
}t_uart_para;
...
{
    unsigned char mask;
    unsigned char com_port;           //连接系统控制板的串口编号
    unsigned char mbus_id;            //modbus_id
    unsigned char SN[22];	    				//Serial Number
    
    unsigned short bat_soc;        		//State of Charge
    unsigned short bat_vol;        		//Battery Voltage
    signed short bat_cur;        			//Current   需要正负值表示吗？
   
    unsigned short mini_cell_vol;  		//Minimum Cell Voltage
    signed short   mini_cell_temp; 		//Minimum Cell Temperature
    unsigned short max_cell_vol;   		//Max Cell Voltage
    signed short   max_cell_temp; 	 	//Max Cell Temperature 
    unsigned short bat_life_time; 	 	//Lifetime
    unsigned short nominal_cap;    		//Nominal Capacity
    unsigned short cycle_cnt;     	 	//Cycle Count
    unsigned short bat_alarm;     	 	//Alarm
    
    unsigned int life_time;  					//Lifetime_hi  second
    
}SLB48_PARA;

///-------------------- MQTT --------------------------------------------------------------------------------------------------------------
typedef struct{
	unsigned int	 uiFlag_Connect_Register;
	unsigned int   uiPacketID;										//报文id  40 02 00 00(20 02 00 00)
	unsigned int   uiRevTimeOutMs;								//从发送到接收数据超时时间		systickCount
  unsigned int   uiSendTickStart;								//发送开始时刻  ms						systickCount
	unsigned char  ucNum;													//con auth data   表示第几条命令  和 ucFlag2配合用
	unsigned char  ucFlag2;												//1=data  2=con   3=ping  4=time  5=auth  		命令发送类型
	unsigned int   uiFlag_Connect;								//连接标志
	unsigned int   uiFlag_Register;								//注册标志
	unsigned char  ucTotal_Upload_Device;	 				//连接设备的总数 yzq add 2023-09-06
	unsigned char  ucID_Upload_Flag[64][3];				//0=连接，1=注册，2=数据传输  标志
	unsigned char  ucID_Upload_Device_Buf[64][13];//yzq add 2023--09-06，连接设备的ID，12位ASCII，[标配]：
																																																		 ...
																																																		 //+1个空调
																																																		 //+1个逆变器
	unsigned short usTm_Interval_Con;			//连接注册时间间隔,单位秒
	unsigned short usTm_Interval_PINGREQ;	//心跳时间间隔,单位秒
	unsigned int   usTm_PINGREQ_Count;		//心跳计时,单位秒
	...
	unsigned short usData_Setup;//要设置的数据
	unsigned short usAddr_Setup_Reg;//要设置的寄存器地址
}struct_mqtt;
